...
export const TableManagementScreen = () => {
  ...
  const confirmSeat = () => {
    ...
    setShowModal(false);
    Alert.alert('�xito', `Mesa ${selectedTable.number} ocupada por ${customerName}`);
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      <View style={{
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          ...
          <Text style={{ fontSize: 12, color: theme.colors.textMuted }}>Total Mesas</Text>
        </View>
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={{ fontSize: 28, fontWeight: '700', color: '#10B981' }}>
            {utilization.total - utilization.occupied}
          </Text>
          <Text style={{ fontSize: 12, color: theme.colors.textMuted }}>Disponibles</Text>
        </View>
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={{ fontSize: 28, fontWeight: '700', color: '#EF4444' }}>
            {utilization.occupied}
          </Text>
          <Text style={{ fontSize: 12, color: theme.colors.textMuted }}>Ocupadas</Text>
        </View>
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={{ fontSize: 28, fontWeight: '700', color: '#F59E0B' }}>
            {utilization.percentage.toFixed(0)}%
          </Text>
          <Text style={{ fontSize: 12, color: theme.colors.textMuted }}>Ocupaci�n</Text>
        </View>
      </View>
      ...
      <View style={{
        ...
        {[
          ...
          <View key={item.status} style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 11, color: theme.colors.textMuted }}>{item.label}</Text>
          </View>
        ))}
      </View>
      ...
      <Modal visible={showModal} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{
            ...
            <Text style={{ fontSize: 12, color: theme.colors.textMuted, marginBottom: 4 }}>NOMBRE DEL CLIENTE</Text>
            <TextInput
              style={{
            ...
            <Text style={{ fontSize: 12, color: theme.colors.textMuted, marginBottom: 4 }}>TAMA�O DEL GRUPO</Text>
            <TextInput
              style={{
                borderWidth: 1, borderColor: theme.colors.border, borderRadius: 8,
                padding: 12, fontSize: 16, marginBottom: 16,
