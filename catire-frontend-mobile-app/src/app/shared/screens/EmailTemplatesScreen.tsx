        ...
        {TEMPLATES.map((template) => (
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 16, fontWeight: '700', color: colors.textPrimary }}>{template.title}</Text>
                <Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 2 }}>{template.description}</Text>
              </View>
              <Switch
                value={toggles[template.id]}
                onValueChange={() => toggleSwitch(template.id)}
                trackColor={{ false: colors.border, true: '#86EFAC' }}
                thumbColor={toggles[template.id] ? '#059669' : colors.borderLight}
              />
            </View>
            <View style={{
              ...
              padding: 12,
              borderLeftWidth: 3,
              borderLeftColor: toggles[template.id] ? '#059669' : colors.border,
            }}>
              <Text style={{ fontSize: 11, color: colors.textMuted, fontWeight: '600', marginBottom: 4 }}>VISTA PREVIA</Text>
              <Text style={{ fontSize: 13, color: colors.textSecondary, lineHeight: 18 }}>{template.preview}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
